...
def risk():
    ...
    answer1 = get_answer(
        "how many attackers in the current roll?", (1, 2, 3)
    )
    answer2 = get_answer(
        "how many defenders in the current roll?", (1, 2)
    )
...
def get_answer(question, choices):
    as_strings = [str(choice) for choice in choices]
    joined_choices = "/".join(as_strings)

    while True:
        answer = input("{} ({})".format(question, joined_choices))

        if answer in as_strings:
            return int(answer)
        else:
            print("wrong answer! choices are: {}".format(joined_choices))
